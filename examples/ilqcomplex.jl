...
nx, nu, ΔT, game_horizon = 6, 3, 0.1, 20
...
function update_target(x, current_target, target_centers)
    ...
    if dot((SVector(x[1], x[3], x[5]) - target_pos), normal_vec) >= 0
        if current_target[1] == length(target_centers)
            current_target[1] = 1
        else
...
function dist_to_hyperplane(x, target_center, norm_vec)
    dist = abs(dot(x - target_center, norm_vec)) / norm(norm_vec)
    return dist
end

function normalize_velocity(x)
    velocity = SVector(x[2], x[4], x[6])
    speed = norm(velocity)
    if speed <= 1
        return x
    end
    vel_norm = velocity / speed
    return [x[1], vel_norm[1], x[3], vel_norm[2], x[5], vel_norm[3]]
end
...
function drone_cost_function(current_target, target_centers, x)
    return (
        FunctionPlayerCost((g, x, u, t) -> (
            ...
            120 * (dist_to_hyperplane(SVector(x[1], x[3], x[5]), target_centers[current_target[1]], normal_vectors[current_target[1]]) < 0.1) *
            ((x[1] - target_centers[current_target[1]][1])^2 + (x[3] - target_centers[current_target[1]][2])^2 + (x[5] - target_centers[current_target[1]][3])^2) +
            (dot((SVector(x[1], x[3], x[5]) - target_centers[current_target[1]]), normal_vectors[current_target[1]]) >= 0) * 
            ((x[1] - target_centers[(current_target[1]%5)+1][1])^2 + (x[3] - target_centers[(current_target[1]%5)+1][2])^2 + (x[5] - target_centers[(current_target[1]%5)+1][3])^2) + 
            0.5 * (u[1]^2 + u[2]^2 + u[3]^2)  # Penalize control effort
            # Speed Control
            #(norm(SVector(x[2], x[4], x[6])) - 1)^2
        )), 
        # FunctionPlayerCost((g, x, u, t) -> (
...
while length(trajectory) < maximum_horizon
    ...
    x_norm = normalize_velocity(full_trajectory.x[2])

    # global x_prev = collect(full_trajectory.x[1])
    global x0 = collect(x_norm)  # Update the state after each step
